...
const TableRowComponent: FC<TableProps> = ({ item, data, menu, path, fields = [], selectable }) => {
	return (
		...
		<TableRow selectable={true} id={item._id} key={item._id} actions={<div></div>}>
			...
			{data.map(({ dataKey, type, image, imageKey, editable, editType, options, style }) => {
				...
				if (type == 'menu')
					return (
						<TableMenu path={path} data={menu} id={item?._id} key={dataKey} title={item[dataKey]} />
					);
				...
				return (
					<TableData key={dataKey} type={type} imageKey={item[imageKey]}>
						{value}
					</TableData>
				);
			})}
