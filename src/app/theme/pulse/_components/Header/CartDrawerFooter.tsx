...
const CartDrawerFooter: FC<CartDrawerFooterProps> = ({ subTotal, basic, css }) => {
	const colors = useColors();

	return (
		<DrawerFooter
			mt={'auto'}
			bg={css?.footerBg || colors?.footerBg}>
			<Box w='full'>
				<Center
					w='full'
					...
					<Box>
						<NormalText
							fontSize={css?.fTextSize}
							fontWeight={css?.fTextWeight}
							basic={basic}>{`${currency?.symbol} ${subTotal?.toLocaleString()}`}</NormalText>
					</Box>
				</Center>
				<Link href='/checkout'>
					<Button
