...
const ProductListTwo: FC<HomeContentProps> = ({ dataModel, content, basic, path, data }) => {
	// const productList = sortDataByPriority(content?.productListTwo);
	const productList =
		content?.productListTwo?.length > 0 && sortDataByPriority(content?.productListTwo);
	// const { basic } = data;
	...
	return (
		<HoverContentContainer
			...
			<AddProductListModal
				dataModel={dataModel}
				path='pulse'
				productListKeys='productListTwo'>
				<Center
					mx={6}
					cursor='pointer'
