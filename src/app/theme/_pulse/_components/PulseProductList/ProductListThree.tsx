...
import { Heading, Text, Flex, Center, FlexProps, TextProps } from '@chakra-ui/react';
import {
	Column,
	Button,
	AddProductListModal,
...
const ProductListThree: FC<HomeContentProps> = ({ dataModel, content, basic, path, data }) => {
	const productList =
		content?.productListThree?.length > 0 && sortDataByPriority(content?.productListThree);
	// const { basic } = data;
	...
	return (
		<HoverContentContainer
			...
			<AddProductListModal
				dataModel={dataModel}
				path='pulse'
				productListKeys='productListThree'>
				<Center
					mx={6}
					cursor='pointer'
